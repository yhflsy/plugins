...
class Builder
{
    ...
    protected function generateHeadersTemplate($id, $st_params)
    {
        ...
        foreach ($st_params['ApiHeaders'] as $params) {
            $tr = array(
                '{{ name }}'        => $params['name'],
                '{{ type }}'        => $params['type'],
                '{{ nullable }}'    => @$params['nullable'] == '1' ? '' : '<span class="glyphicon glyphicon-ok"></span>',
    ...
    protected function generateParamsTemplate($id, $st_params)
    {
        ...
        foreach ($st_params['ApiParams'] as $params) {
            $tr = array(
                '{{ name }}'        => $params['name'],
                '{{ type }}'        => $params['type'],
                '{{ nullable }}'    => @$params['nullable'] == '1' ? 'No' : '<span class="glyphicon glyphicon-ok"></span>',
                '{{ description }}' => @$params['description'],
            );
    ...
    private function generateBodyTemplate($id, $docs)
    {
        ...
        $body = "";
        if (is_array($docs['ApiBody'][0]) && $docs['ApiBody'][0]['sample']) {
            parse_str($docs['ApiBody'][0]['sample'], $data);
            if (! empty($data)) {
